...
filename2 = f"Features_{query}_{instrument}_localnorm.png"
plt.savefig(filename2)
plt.show()

#correction funtions -- help of GEMINI AI

def get_centroid(wave, flux, line_center, width=3):
    mask = (wave > line_center - width) & (wave < line_center + width)
    w, f = wave[mask], flux[mask]
    if len(w) < 3: return None
    weight = np.max(f) - f 
    return np.sum(w * weight) / np.sum(weight)

def local_normalize(x, y, line_center, window=25):
    mask = (x > line_center - window) & (x < line_center + window)
    w_sub, f_sub = x[mask], y[mask]
    shoulder_mask = (w_sub < line_center - 3) | (w_sub > line_center + 3)
    fitter = fitting.LinearLSQFitter()
    p = fitter(models.Polynomial1D(1), w_sub[shoulder_mask], f_sub[shoulder_mask])
    return w_sub, f_sub / p(w_sub)

#calculating shift

h_alpha_vacc = 6564.61 
# IAU Air Conversion
s = 10**4 / h_alpha_vacc
n = 1 + 0.0000834254 + 0.02406147 / (130 - s**2) + 0.00015998 / (38.9 - s**2)
h_alpha_ref = h_alpha_vacc / n

w_ha, f_ha = local_normalize(xvals, yvals, h_alpha_ref)
observed_center = get_centroid(w_ha, f_ha, h_alpha_ref)

# Calculate Shift and Velocity
offset = observed_center - h_alpha_ref
velocity_km_s = (offset / h_alpha_ref * const.c.to('km/s')).value


#plots with correction
line_configs = {
...
plt.suptitle(f'Key Spectral Features for {query}\n(Dynamically Corrected: RV = {velocity_km_s:.2f} km/s)', fontsize=16)
plt.subplots_adjust(hspace=0.4, wspace=0.3)

for ax, (name, config) in zip(axes.flatten(), line_configs.items()):
    vac_wave = config['vac']
    
    # Calculate expected air wavelength for THIS specific star
    s_l = 10**4 / vac_wave
    n_l = 1 + 0.0000834254 + 0.02406147 / (130 - s_l**2) + 0.00015998 / (38.9 - s_l**2)
    expected_air = (vac_wave / n_l) + offset
    
    try:
        # Extract and normalize
        w_p, f_p = local_normalize(xvals, yvals, expected_air, window=25)
        
        # Plotting
        ax.plot(w_p, f_p, color=config['color'], lw=1.5)
        ax.axvline(expected_air, color='black', linestyle='--', alpha=0.7, label='Corrected Center')
        ax.axhline(y=1.0, color='gray', ls=':', linewidth=0.8)
        
        # Formatting
        ax.set_title(f"{config['title']} ({vac_wave} Å)")
        ax.set_xlabel('Wavelength (Å)')
        ax.set_ylabel('Normalized Flux')
        
        # Auto-scaling y-axis buffer
        pad = 0.1 * (np.nanmax(f_p) - np.nanmin(f_p))
        ax.set_ylim(np.nanmin(f_p) - pad, np.nanmax(f_p) + pad)
        
    except Exception as e:
        ax.text(0.5, 0.5, f"Line Out of Range\n{vac_wave}Å", ha='center', va='center', transform=ax.transAxes)
