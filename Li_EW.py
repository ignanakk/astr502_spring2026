...
if wave_li2 is not None:
    # Find the actual line minimum near the expected Li position
    ...
    # Fe I shifts by same residual offset
    ax.axvline(x=6707.4 + residual_shift, color='red', ls=':',
               label=f'Fe I (shifted): {6707.4 + residual_shift:.3f} Å')
    ax.axhline(y=1.0, color='gray', ls=':', linewidth=0.8)
...
ax.set_title(f'Lithium 6707.76 Å — HATS-34\n'
             f'Detected center: {detected_li_center:.3f} Å  (residual offset: {residual_shift:+.3f} Å)')
ax.set_xlabel('Wavelength (Å)')
ax.set_ylabel('Normalized Flux')
plt.tight_layout()
plt.show()

# Apply residual correction on top of the SIMBAD RV correction
xvals_rest2 = xvals_rest / (1 + (residual_shift / 6707.76) * (c_kms / c_kms))
# Simpler: just subtract the shift directly from the wavelength array
...
def ew_diagnostic(wavelength, flux, li_center=6707.8, window=6.0, search_window=1.5,
    ...
    # Continuum: find ALL absorption lines in the window and exclude them
    # First pass — identify any pixels more than 2% below local median as line cores
    local_median = np.median(fl)
    absorption_mask = fl < (local_median * 0.98)  # exclude pixels in any absorption dip

    edge = (
        ((wl < detected_center - window/3) | (wl > detected_center + window/3))
        & ~absorption_mask  # exclude ALL absorption features, not just the Li line
    )

    if np.sum(edge) < 4:
        print("  Warning: too few continuum anchor points, relaxing absorption threshold.")
        absorption_mask = fl < (local_median * 0.94)
        edge = (
            ((wl < detected_center - window/3) | (wl > detected_center + window/3))
            & ~absorption_mask
        )
    ...
    # EW: trapezoid integration over narrow window around detected center
    int_mask = (wl > detected_center - ew_half_width) & (wl < detected_center + ew_half_width)
    wl_int = wl[int_mask]
    fl_int = fl[int_mask]
    ...
    fig.suptitle(f'Li EW Diagnostic — EW = {ew_val:.1f} mÅ\n'
...
li_ew, li_detected = ew_diagnostic(wave_li2, flux_li2, 
                                   li_center=detected_li_center,  # already found above
                                   window=6.0, 
                                   search_window=0.3,  # tight — just confirms the minimum nearby
                                   ew_half_width=0.5)
print(f'Lithium EW: {li_ew:.3f} mÅ  (line detected at {li_detected:.3f} Å)')